...
interface ResourceCard { title: string; text: string; href: string; button: string; visible: boolean }

export default function ResourceArchitectureEditor() {
  const { fields, updatedAt, loading, saving, error, notice, save } = useSection("homepage.resources");
  const [vals, setVals] = useState<Record<string, string>>({});
  const [cards, setCards] = useState<ResourceCard[]>([]);

  const SCALARS = ["label", "heading", "description"];
  ...
  useEffect(() => {
    if (fields) {
      ...
      setVals(v);
      setCards(Array.isArray(fields.cards) ? (fields.cards as ResourceCard[]) : []);
    }
  }, [fields]);

  const setCard = (i: number, patch: Partial<ResourceCard>) => setCards((cs) => cs.map((x, j) => (j === i ? { ...x, ...patch } : x)));
  const addCard = () => setCards((cs) => [...cs, { title: "New resource", text: "", href: "/resources", button: "Learn more", visible: true }]);
            ...
            <div className="space-y-4">
              {cards.map((card, i) => (
                <div key={i} className={`rounded-xl border p-4 ${card.visible ? "border-[#e8eef5] bg-[#fbfdff]" : "border-[#e8eef5] bg-[#f3f4f6] opacity-75"}`}>
                  ...
                  <div className="mt-3"><Field label="Card description"><textarea value={card.text} onChange={(e) => setCard(i, { text: e.target.value })} rows={2} className={inputCls + " resize-y"} /></Field></div>
                  <div className="mt-3 grid gap-3 sm:grid-cols-2">
                    <Field label="Button text"><input value={card.button} onChange={(e) => setCard(i, { button: e.target.value })} className={inputCls} /></Field>
