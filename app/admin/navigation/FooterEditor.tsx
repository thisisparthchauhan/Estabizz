...
export default function FooterEditor() {
  ...
  // column helpers
  const setColumn = (ci: number, patch: Partial<FooterColumn>) => { setColumns((cs) => cs.map((c, i) => (i === ci ? { ...c, ...patch } : c))); touched(); };
  const setColLink = (ci: number, li: number, patch: Partial<FooterLink>) => { setColumns((cs) => cs.map((c, i) => i === ci ? { ...c, links: c.links.map((l, j) => (j === li ? { ...l, ...patch } : l)) } : c)); touched(); };
  const addColLink = (ci: number) => { setColumns((cs) => cs.map((c, i) => i === ci ? { ...c, links: [...c.links, { label: "", href: "" }] } : c)); touched(); };
  const removeColLink = (ci: number, li: number) => { setColumns((cs) => cs.map((c, i) => i === ci ? { ...c, links: c.links.filter((_, j) => j !== li) } : c)); touched(); };
  const addColumn = () => { setColumns((cs) => [...cs, { title: "New Column", links: [{ label: "", href: "" }] }]); touched(); };
  const removeColumn = (ci: number) => { setColumns((cs) => cs.filter((_, i) => i !== ci)); touched(); };

  // regulator helpers
  const setReg = (ri: number, patch: Partial<FooterLink>) => { setRegulators((rs) => rs.map((r, i) => (i === ri ? { ...r, ...patch } : r))); touched(); };
  const addReg = () => { setRegulators((rs) => [...rs, { label: "", href: "" }]); touched(); };
    ...
    <div className="space-y-6">
      ...
      <div className="flex justify-end">
        <button onClick={handleSave} disabled={saving} className="rounded-xl bg-[#1677f2] px-6 py-2.5 text-[13px] font-bold text-white transition-all hover:bg-[#0f63d6] disabled:cursor-not-allowed disabled:opacity-50">
          {saving ? "Saving…" : "Save & Publish Footer"}
        </button>
      </div>
    </div>
  );
