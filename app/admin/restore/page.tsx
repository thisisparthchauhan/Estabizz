...
export default async function RestorePage() {
  ...
  const initialVersions = viewer
    ? await listRestoreVersions(viewer, { page: 1, limit: 25, type: "all" })
    : { items: [], page: 1, limit: 25, total: 0, totalPages: 1 };

  return <RestoreClient initialVersions={initialVersions} viewerEmail={viewer?.email ?? ""} />;
}
