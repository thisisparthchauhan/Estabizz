        ...
        <section ref={sectionRef} className="bg-white py-24">
            <div className="mx-auto max-w-[1240px] px-6">
                <div className={`mx-auto mb-14 max-w-3xl text-center transition-all duration-700 ${isVisible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-8"}`}>
                    <div className="mb-4 text-[12px] font-black uppercase tracking-[0.18em] text-[#0096D6]">Resource Architecture</div>
                ...
                <div className="grid grid-cols-1 gap-6 md:grid-cols-3">
                    {resources.map((resource, index) => (
                        <Link
                            key={resource.title}
                            href={resource.href}
                            className={`group rounded-2xl border border-blue-100 bg-[#fbfdff] p-7 shadow-sm transition-all duration-500 hover:-translate-y-1 hover:border-[#0096D6]/40 hover:shadow-[0_18px_45px_rgba(0,100,200,0.12)] ${isVisible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-8"}`}
                        ...
                        >
                            ...
                            <span className="text-[14px] font-bold text-[#0096D6] group-hover:underline">{resource.button} →</span>
                        </Link>
                    ))}
                </div>
            </div>
