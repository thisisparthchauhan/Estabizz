            ...
            <section className="border-b border-blue-100 px-6 py-16" style={{ background: "linear-gradient(135deg,#eff6ff,#f8fbff,#fffaf0)" }}>
                <div className="mx-auto max-w-7xl">
                    <nav className="mb-6 flex gap-2 text-sm font-medium text-gray-500">
                        <Link href="/" className="hover:text-[#0096D6]">Home</Link><span>&gt;</span><Link href="/resources" className="hover:text-[#0096D6]">Resources</Link><span>&gt;</span><span className="text-[#0096D6]">Service Page Framework</span>
                    </nav>
                    <span className="mb-4 inline-block rounded-full border border-blue-100 bg-white px-4 py-2 text-xs font-black uppercase tracking-[0.16em] text-[#0096D6]">SEO + GEO + AEO Content Engine</span>
                    <h1 className="max-w-5xl text-[38px] font-black leading-tight text-[#0a1628] md:text-[52px]">Service Page Content Framework for Regulatory Licence Pages</h1>
            ...
            <div className="mx-auto grid max-w-7xl grid-cols-1 gap-8 px-6 py-14 lg:grid-cols-[280px_1fr]">
                <aside className="hidden lg:block">
                    <div className="sticky top-24 rounded-2xl border border-blue-100 bg-white p-5 shadow-sm">
                        <p className="mb-4 text-xs font-black uppercase tracking-[0.16em] text-[#0096D6]">Framework Flow</p>
                        {frameworkSections.slice(0, 14).map((section) => (
                            <a key={section} href={`#${section.replaceAll(" ", "-").replaceAll("/", "").toLowerCase()}`} className="block rounded-lg px-3 py-2 text-[13px] font-bold text-[#475569] hover:bg-blue-50 hover:text-[#0096D6]">{section}</a>
                        ))}
                    </div>
                </aside>
                        ...
                        <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
                            {[
                                ...
                                <div key={step} className="rounded-2xl border border-blue-100 bg-[#f8fbff] p-5">
                                    <span className="text-xs font-black text-[#0096D6]">Step {index + 1}</span>
