...
import { getGovLicService, getAllGovLicSlugs } from "@/lib/gov-lic";
...
export async function generateStaticParams() {
  return getAllGovLicSlugs().map((slug) => ({ slug }));
}

export async function generateMetadata({ params }: Props): Promise<Metadata> {
  const { slug } = await params;
  const svc = getGovLicService(slug);
  if (!svc) return { title: "Not Found", robots: { index: false, follow: false } };
  return {
    title: svc.seoTitle,
    description: svc.metaDescription,
    alternates: { canonical: `/gov-lic/${svc.slug}` },
    openGraph: {
      title: svc.seoTitle,
      description: svc.metaDescription,
      type: "website",
      url: `/gov-lic/${svc.slug}`,
...
export default async function GovLicServicePage({ params }: Props) {
  const { slug } = await params;
  const svc = getGovLicService(slug);
  ...
  const breadcrumbSchema = {
    ...
    itemListElement: [
      ...
      { "@type": "ListItem", position: 2, name: "Government Licences", item: `${SITE_URL}/gov-lic` },
      { "@type": "ListItem", position: 3, name: svc.title, item: `${SITE_URL}/gov-lic/${svc.slug}` },
    ],
  };
