...
| ~~RL-003~~ | ~~No rate limiting on `POST /api/auth/signup`, `/api/leads`, `/api/submit-blog`~~ | ~~Medium — spam/DB abuse~~ | **RESOLVED 2026-07-23** — Signup: 8 KB body, 5/IP/15 min + 3/email/hr. Leads: 16 KB body, 5/IP/hr + 3/email/24 hr. Submit-blog: 128 KB body, 3/IP/hr + 2/email/12 hr. All fail-open, isRateLimitConfigured() gate, arrayBuffer() byte enforcement, Cache-Control: no-store on errors. |
