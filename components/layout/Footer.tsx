...
export default function Footer() {
    return (
        <footer className="relative overflow-hidden bg-[#0a1628] text-white/70">
            ...
            <div className="relative z-10 mx-auto max-w-[1400px] px-6 pb-3 pt-5">
                <div className="grid grid-cols-1 gap-8 xl:grid-cols-[280px_1fr]">
                    {/* Brand */}
                    <div>
                        <Link href="/" className="group mb-2 flex items-center gap-2.5">
                            <Image src="/estabizz-logo-light.png" alt="Estabizz" width={747} height={314} className="h-8 w-auto transition-transform group-hover:scale-[1.03]" />
                            <div className="leading-tight">
                                <div className="text-[17px] font-black tracking-tight text-white">Fintech</div>
                                <div className="text-[10px] font-bold text-[#4f9dfb]">We Comply. We Simplify.</div>
                            </div>
                        </Link>

                        <p className="max-w-[280px] text-[12px] leading-relaxed text-white/55">
                            Structured regulatory advisory and compliance infrastructure partner for Indian and global businesses.
                        </p>

                        <div className="mt-2 space-y-1.5">
                            <div className="rounded-lg border border-white/10 bg-white/[0.04] px-3 py-1.5 text-[11px] leading-snug text-white/65">
                                15, Vedika Exotika Bungalow, Adalaj, Gandhinagar, Gujarat - 382421
                            </div>
                            <a href="tel:+919825600907" className="block rounded-lg border border-white/10 bg-white/[0.04] px-3 py-2 text-[11.5px] font-semibold text-white/70 transition-colors hover:border-[#1677f2]/40 hover:text-[#4f9dfb]">
                                +91 98256 00907
                            </a>
                            <a href="mailto:info@estabizz.com" className="block rounded-lg border border-white/10 bg-white/[0.04] px-3 py-2 text-[11.5px] font-semibold text-white/70 transition-colors hover:border-[#1677f2]/40 hover:text-[#4f9dfb]">
                                info@estabizz.com
                            </a>
                        </div>
                        ...
                        <div className="mt-3 flex gap-2">
                            {[
                                ...
                                <a key={s.label} href={s.href} target="_blank" rel="noopener noreferrer" title={s.label}
                                    className="flex h-8 w-8 items-center justify-center rounded-full border border-white/12 bg-white/[0.05] text-[11px] font-black text-white/65 transition-all hover:-translate-y-1 hover:border-[#1677f2] hover:bg-[#1677f2] hover:text-white">
                                    {s.icon}
                                </a>
                            ))}
                        </div>

                        <div className="mt-2 text-[10.5px] font-semibold text-white/35">
                            CIN: U74999GJ2021PTC123384
                        </div>
                    </div>

                    {/* Link columns — all 7 in one row at lg+ */}
                    <div className="grid grid-cols-2 gap-x-4 gap-y-5 sm:grid-cols-4 lg:grid-cols-7">
                        {Object.entries(footerLinks).map(([title, links]) => (
                            <div key={title}>
                                <h4 className="mb-2 text-[10px] font-black uppercase tracking-[0.14em] text-[#4f9dfb]">{title}</h4>
                                <ul className="space-y-1">
                                    {links.map((link) => (
                                        <li key={link.label}>
                                            <Link href={link.href} className="inline-block text-[11.5px] font-medium leading-snug text-white/55 transition-all hover:pl-1 hover:text-white">{link.label}</Link>
                                        </li>
                                    ))}
                                </ul>
                            </div>
                ...
                <div className="mt-5 flex flex-wrap items-center gap-1.5 border-t border-white/10 pt-4">
                    <span className="mr-2 text-[10px] font-black uppercase tracking-[0.18em] text-white/40">Regulators:</span>
                    {regulators.map((r) => (
                        <Link key={r.label} href={r.href} className="rounded-full border border-white/12 bg-white/[0.05] px-3 py-1 text-[10px] font-bold text-white/70 transition-all hover:-translate-y-0.5 hover:border-[#1677f2] hover:bg-[#1677f2] hover:text-white hover:shadow-[0_8px_20px_rgba(22,119,242,0.3)]">
                            {r.label}
                        </Link>
                    ))}
                </div>
                ...
                <div className="mt-2 flex flex-col items-center justify-between gap-2 border-t border-white/10 pt-2 text-[11px] font-medium text-white/40 md:flex-row">
