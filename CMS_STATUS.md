...
> Last updated: 2026-06-26 (IST) · Phase: **4L — Controlled RBI Batch-1 Expansion** · Last batch: **4L controlled RBI batch expansion**
...
- **Public Content Pages Controlled RBI Batch-1 Expansion (Phase 4L):** exactly two additional RBI guide pages were added to the CMS-managed set: `/rbi/payment-aggregator-license-in-india` and `/rbi/ppi-registration-in-india`. Together with `/rbi/nbfc-registration-in-india`, `public_content_pages` now contains exactly three published CMS-managed records. DB-first rendering is connected only for those three paths; unmanaged pages continue to render from their hardcoded source. If a managed DB record is not published, public rendering blocks fallback and returns not found. The editor uses whitelist-locked routes (`/admin/content-pages/[slug]/edit`), APIs accept only the three managed paths, Approval Queue shows pending changes for those paths only, Backup includes only the three managed CMS records, and Recycle Bin supports soft delete/restore only while permanent purge remains blocked. Broad import apply is blocked; targeted dry-runs now skip existing records. No navbar, mega menu, homepage CMS, regulatory update, Sales CRM, Client Ticketing, Blog redesign, GitHub push, or production push was started. Phase 4M is not started.
...
12. **Content Pages — DONE (Inventory 4C + Sample Renderer/Baseline 4D/4E + Editor/Lifecycle 4F-4K + Controlled RBI Batch 4L)** — `/admin/content-pages`. Inventory reviews discovered public content pages with summary counts, search, filters, open-page links and copy path. Exactly three RBI guide paths are CMS-managed: `/rbi/nbfc-registration-in-india`, `/rbi/payment-aggregator-license-in-india`, and `/rbi/ppi-registration-in-india`. Those paths use DB-first rendering, whitelist-locked visual editors, Pending Changes save, shared Approval Queue review, controlled media/design presets, Recycle Bin soft delete/restore, and Backup coverage. Unmanaged public pages remain hardcoded and do not show active editors. Permanent purge remains blocked for content pages.
---
...
**Phase 4L — DONE:** Controlled RBI Batch-1 Expansion. Selected exactly two safe RBI guide pages for the first batch: `/rbi/payment-aggregator-license-in-india` and `/rbi/ppi-registration-in-india` (both guide pages, menu group `regulatory`, regulator `RBI`, rendered through `ServicePageLayout`). Imported exactly those two pages with targeted `--apply --only=...`, leaving `public_content_pages` at exactly three published CMS-managed records including `/rbi/nbfc-registration-in-india`. Added `PUBLIC_CONTENT_MANAGED_PATHS` as the whitelist for editor routes, editor APIs, approval/reject/delete APIs, Approval Queue, Recycle Bin listing/restore and Backup. Connected DB-first public rendering only for the three managed paths; non-managed pages keep hardcoded rendering. Pending changes remain private until approval, self-review is blocked, rejected changes do not publish, and public pages ignore pending fields. Recycle Bin supports soft delete/restore for the three managed pages; permanent purge remains blocked. Backup includes exactly the three managed CMS records. Broad import apply is blocked; targeted dry-runs after import skip existing records. No other pages were migrated, navbar/mega menu/homepage CMS/regulatory update behavior was unchanged, and Phase 4M was not started.
**Next after owner approval:** Phase 4M or further public content page migration only after explicit owner approval. Do not start Sales CRM, Client Ticketing, Regulatory Update Desk work, Reports & Analytics or Blog redesign. Do not push anywhere until MongoDB password rotation is complete.
